...
INSTALL_ALL_FORCE=true
SPI_DRIVER_FORCE=SPIDEV
...
if [ -n "$SPI_DRIVER_FORCE" ]; then
	echo "Skipping interactive option"
	RF24DRIVER+=$SPI_DRIVER_FORCE
else
...
install_repo() {
	...
	if [ -n "$INSTALL_ALL_FORCE" ]; then
		...
		read -p $'\n'"Do you want to build the ${REPOS[$1]} examples [Y/n]? " answer=Y
	fi
