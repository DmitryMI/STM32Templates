...
const osThreadAttr_t mainTask_attributes = { .name = "mainTask", .stack_size = 3000 * 4, .priority = (osPriority_t) osPriorityNormal, };
...
static void MX_GPIO_Init(void)
{
	...
	HAL_GPIO_WritePin(RF24_CSN_GPIO_Port, RF24_CSN_Pin, GPIO_PIN_RESET);

	/*Configure GPIO pin Output Level */
	HAL_GPIO_WritePin(RF24_CE_GPIO_Port, RF24_CE_Pin, GPIO_PIN_RESET);
	...
	GPIO_InitStruct.Pull = GPIO_NOPULL;
	HAL_GPIO_Init(RF24_INT_GPIO_Port, &GPIO_InitStruct);
